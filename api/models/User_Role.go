...
func (ur *User_Role) DeleteUserFromRole(db *gorm.DB, rid uint32, uid uuid.UUID) (int64, error) {
...
func (ur *User_Role) DeleteUsersFromRole(db *gorm.DB, rid uint32, uids []uuid.UUID) int64 {

	totalRowsAffected := int64(0)

	for i := range uids {
		rowsAffected := int64(0)
		rowsAffected, _ = ur.DeleteUserFromRole(db, rid, uids[i])
